...
│  │ 分析需求 │───────────────▶│ 规划任务 │───────────────▶           │
│  │  ~2min  │    Save@Step 3    │  ~3min  │    Save@Step 7          │
│  └─────────┘                   └─────────┘                          │
│       ▲                                                           │
...
│  │                     State Management                         │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐        │   │
│  │  │  State Snapshot│ │  State Diff │  │ State Version│        │   │
│  │  │  (Full Copy)  │ │  (Changes)   │  │  (Tracking)  │        │   │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘        │   │
│  └─────────┼─────────────────┼─────────────────┼─────────────────┘   │
│            │                 │                 │                    │
│            ▼                 ▼                 ▼                    │
...
│  │  ┌─────────────────────────────────────────────────────────┐ │   │
│  │  │                   Checkpointer                          │ │   │
│  │  │  - get(thread_id, checkpoint_id?) -> Checkpoint          │ │   │
│  │  │  - put(thread_id, checkpoint) -> checkpoint_id           │ │   │
│  │  │  - list(thread_id) -> List[checkpoint_id]                │ │   │
...
│        ▼                                                            │
│  ┌─────────────────┐                                               │
│  │  Create Thread   │ ───▶ thread_id                                │
│  └────────┬────────┘                                               │
│           │                                                         │
│           ▼                                                         │
│  ┌─────────────────┐     ┌─────────────────┐     ┌───────────────┐ │
│  │   Run Step 1    │────▶│   Run Step 2    │────▶│   Run Step N  │ │
│  │  Save Checkpoint │     │  Save Checkpoint │     │  Save Checkpoint│ │
│  │    id: cp_001    │     │    id: cp_002    │     │   id: cp_N    │ │
│  └────────┬────────┘     └────────┬────────┘     └───────┬───────┘ │
│           │                       │                       │         │
│           ▼                       ▼                       ▼         │
│    ┌────────────┐          ┌────────────┐          ┌────────────┐ │
│    │ Checkpoint │◀─────────│ Checkpoint │◀─────────│ Checkpoint │ │
│    │   cp_001   │  parent  │   cp_002   │  parent  │   cp_N     │ │
│    └────────────┘          └────────────┘          └────────────┘ │
│         │                       │                       │          │
│         └───────────────────────┴───────────────────────┘          │
│                             │                                       │
...
│  │                    恢复/回溯操作                              │   │
│  │                                                             │   │
│  │   get_state(thread_id, checkpoint_id="cp_002")             │   │
│  │   update_state(thread_id, state)                            │   │
│  │   replay(thread_id, checkpoint_id)                         │   │
│  │                                                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
```python
# 线程隔离示意图
Thread A (user_1)          Thread B (user_2)          Thread C (user_3)
┌─────────────────┐        ┌─────────────────┐        ┌─────────────────┐
│ cp_a_001        │        │ cp_b_001        │        │ cp_c_001        │
│   └─ cp_a_002   │        │   └─ cp_b_002   │        │   └─ cp_c_002   │
│       └─ cp_a_003│        │       └─ cp_b_003│        │       └─ cp_c_003│
└─────────────────┘        └─────────────────┘        └─────────────────┘

# 每个线程有独立的检查点链，互不干扰
# 存储: thread_id + checkpoint_id 作为复合键
```
